...
# The result is placed in dist\package\Appointment.exe.
...
Copy-Item -Path "$root\backend\dist\main.exe" -Destination "$packageOutput\Appointment.exe"
Copy-Item -Path "$root\README.md" -Destination "$packageOutput\README.md"

Write-Host 'Package ready:'
Write-Host "  $packageOutput\Appointment.exe"
